...
# async def create_product(product: ProductCreate, session: Annotated[Session, Depends(get_session)], current_user: Annotated[User, Depends(is_user_vendor)]):
#     product_obj = Product(**dict(product), created_at=datetime.datetime.now(datetime.UTC), vendor=current_user)
#     try:
#         product_dict = Product.model_validate(product_obj, strict=True)

#         current_user.products.append(product_dict)
