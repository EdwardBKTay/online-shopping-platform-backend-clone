...
from db.models import Cart, Product, User, CartItem, Order, OrderItem, OrderRead
...
async def checkout_cart(username: str, session: Annotated[Session, Depends(get_session)], current_user: Annotated[User, Depends(is_only_user)]):
    ...
    for cart_item in cart_items:
        ...
        order_item = OrderItem(quantity=cart_item.quantity, product=product, order=user_order, created_at=datetime.datetime.now(datetime.UTC)) # type: ignore
    ...
    return OrderRead(**user_order.model_dump())
...
async def add_to_cart(product_id: int, username: str, req: CartCreate, session: Annotated[Session, Depends(get_session)], user: Annotated[User, Depends(is_only_user)]):
    ...
    user_cart = session.exec(select(Cart).where(Cart.user == user)).one_or_none()
    ...
    stmt = select(CartItem).where(CartItem.product == product and CartItem.cart == user_cart)
...
# TODO: allow users to see the prices in the cart
